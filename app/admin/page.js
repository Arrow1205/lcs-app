...
export default function AdminPage() {
  ...
  const exportCSV = (data, filename) => {
    ...
    // On nettoie les objets imbriqués pour le CSV (ex: ventes)
  ...
  const zonesMap = {};
  vendeurs.forEach(v => {
    if (v.zone) zonesMap[v.zone] = (zonesMap[v.zone] || 0) + 1;
  });
      ...
      {loading ? <p style={{textAlign: "center"}}>Chargement...</p> : (
        <div className="animate-fade-in">
          ...
          {activeTab === "vendeurs" && (
            <>
              {/* Camembert des Zones */}
              <div style={{ background: "#fff", borderRadius: 12, padding: 20, marginBottom: 20, border: "1px solid #eee", textAlign: "center" }}>
              ...
              <div style={{ background: "#fff", borderRadius: 12, padding: 20, marginBottom: 20, border: "1px solid #eee" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {top5Acheteurs.map((a, i) => (
                    <div key={a.id} style={{ display: "flex", justifyContent: "space-between", padding: 10, background: i===0?"#FFF5F0":"#f9f9f9", borderRadius: 8, fontWeight: i===0?"bold":"normal" }}>
